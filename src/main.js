...
function updateCartUI() {
	...
	// Visa bulk-rabatter (per kategori)
	...
	if (hasBulkDiscounts) {
		...
		Object.entries(bulkDiscounts).forEach(([cat, amount]) => {
			if (amount > 0) {
				...
				row.innerHTML = `10% bulk discount on ${cat}: -${amount} kr`;
	...
	if (shippingCost > 0) {
		shippingRow.innerHTML = `Shipping: ${shippingCost} kr`;
